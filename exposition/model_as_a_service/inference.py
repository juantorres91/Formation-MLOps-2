...
from exposition.model_as_a_service.config import GENERATED_DATA_PATH, DATA_FOLDER, MODEL_PATH
...
def predict_endpoint():
    ...
    passed_query = request.args.to_dict()
    
    for param_name in passed_query:
        received_data_df[param_name][0] = float(passed_query[param_name])

    features = prepare_features(received_data_df, False)
    predictions = predict(features, MODEL_PATH)
